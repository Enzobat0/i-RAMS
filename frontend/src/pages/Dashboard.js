...
const Dashboard = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans pb-10" style={{ background: 'linear-gradient(135deg, #f0f4f1 0%, #e8f0ea 100%)' }}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5 px-4 sm:px-6 lg:px-10 py-4 sm:py-6">
        ...
        <KPICard title="Damage Index (Simulated)" value={summary?.avg_ddi || "0.0"} unit="DDI" />
      </div>

      {/* Main Workspace */}
      <div className="flex flex-col lg:flex-row flex-1 px-4 sm:px-6 lg:px-10 pb-6 gap-4 sm:gap-6 min-h-0">
        
        <div className="flex-1 flex flex-col gap-6 sm:gap-10 min-w-0">
          {/* Map */}
          <div className="relative h-[350px] sm:h-[450px] lg:h-[600px] rounded-2xl sm:rounded-3xl overflow-hidden border border-slate-200 shadow-lg bg-white">
        ...
        {/* Sidebar detail — full width on mobile, fixed on desktop */}
        {selectedSegment && (
